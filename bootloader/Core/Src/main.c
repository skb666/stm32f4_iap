...
int main(void)
{
  ...
  MX_IWDG_Init();
  /* USER CODE BEGIN 2 */
  boot_param_read_check(&boot_param);
  LL_IWDG_ReloadCounter(IWDG);
  boot_addr = select_boot_addr(&boot_param);
  ...
  while (HAL_GetTick() - tickstart < wait) {
    if (get_WK_UP()) {
      ...
      uart6_printf("force boot to `factory` \n");
      break;
    }
  }
